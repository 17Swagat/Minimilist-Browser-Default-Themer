...
// function View_WebsiteShortcuts() {
  ...
  return (
    ...
    <div className="bg-red-500 w-25 h-25 text-white absolute bottom-3 left-3 flex items-center justify-center z-1">
      ...
      ${webShortcutLinksOpen ? 
        "bg-blue-500 transition-all duration-300 translate-x-[65%] -translate-y-[70%] w-[550%] h-[250%]  flex gap-x-1.5 gap-y-2 flex-wrap place-content-start rounded-2xl p-1  overflow-y-auto" 
        :
        "bg-transparent transition duration-300 translate-x-0 -translate-y-0 scale-x-100 scale-y-100 overflow-visible"}`} 
        onClick={(e)=>{
          e.stopPropagation()
        }}
        >
